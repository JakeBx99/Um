...
namespace RBX_Alt_Manager
{
    public partial class AccountManager : Form
    {
        ...
        private void AccountManager_Load(object sender, EventArgs e)
        {
            ...
            if (PlaceID.Text == "5315046213") PlaceID.Text = "2534724415"; // Migrate old default
            UserID.Text = General.Exists("SavedFollowUser") ? General.Get("SavedFollowUser") : string.Empty;
            LaunchData.Text = General.Exists("SavedLaunchData") ? General.Get("SavedLaunchData") : "{\"psCode\":\"lanrp\"}";
            PlaceTimer.Start(); // Trigger game name lookup on startup
            ...
            if (General.Get<bool>("CheckForUpdates") && !Developer.Get<bool>("DevMode")) // Disabled by default, added dev check just in case
            {
                Task.Run(() =>
                {
                    try
