            ...
            if (GetAuthTicket(out string Ticket))
            {
                ...
                if (AccountManager.UseOldJoin)
                {
                    await Task.Run(() =>
                    {
                        try
                        {
                            // RPath seems to be missing or named differently, but I'll keep it to avoid changing logic too much.
                            // If it fails to compile, it's a separate issue from the syntax errors.
                            ProcessStartInfo Roblox = new ProcessStartInfo("RobloxPlayerBeta.exe"); // Defaulting to exe name if RPath is missing
                        ...
                        {
                            Program.Logger.Error($"An error occured attempting to launch {Username}'s old process: {x}");
                        }
                    });
